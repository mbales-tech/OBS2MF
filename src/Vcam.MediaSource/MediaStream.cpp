...
HRESULT MediaStream::Initialize(IMFMediaSource* source, int index)
{
	...
	// OBS2MF advertises a single fixed NV12 format. NV12 is the camera-native format,
	// is accepted by the target 32-bit app (verified in the spike) and by browsers, and
	// avoids any NV12->RGB32 conversion on the live path. The broker always produces
	// frames at exactly this size, so no negotiation is required.
...
HRESULT MediaStream::SetD3DManager(IUnknown* manager)
{
	...
	// OBS2MF forces the CPU path: the media source injects broker-provided NV12 directly
	// into the sample buffer, so we intentionally do NOT wire up the D3D manager here.
	// (Keeping this a no-op makes HasD3DManager() stay false in the FrameGenerator.)
